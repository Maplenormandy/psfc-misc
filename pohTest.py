...
def getPohFull(efitTree):
    ...
    vsurf = gaussian_filter1d(ssibryNode.data(), 1, order=1, truncate=5.0) / np.median(np.diff(ssibryNode.dim_of().data())) * 6.28
    didt = gaussian_filter1d(np.abs(cpasmaNode.data()), 1, order=1, truncate=5.0) / np.median(np.diff(cpasmaNode.dim_of().data()))
...
efitTree = MDSplus.Tree('efit01', 1150903021)
