...
def raytrace(m, det, m_y, m_z, det_xi, det_zeta, generateRays=False):
    ...
    if generateRays:
        ...
        #print l_dm_m.shape
        #print n_m.shape
        l_mp_m = l_dm_m[:,:,0] - 2*np.outer(n_m[:,0], np.einsum('i...,i...', n_m[:,0], l_dm_m[:,:,0])) # reflection r=d-2(d.n)n
...
def generateRays(shot, module, detSlice, plotting=True):
    mirror, det = getInfoFile(shot, module)
    m_y = np.array([0.0])
    m_z = np.array([0.0])
    
    det_xi_grid, det_zeta_grid = np.meshgrid(np.array(range(det.n_xi)), np.array([detSlice]))
    #det_xi_grid, det_zeta_grid = np.meshgrid(np.array([detSlice]), np.array(range(det.n_zeta)))
    lam1, weight1, rays = raytrace(mirror, det, m_y, m_z, det_xi_grid.flatten(), det_zeta_grid.flatten(), generateRays=True)
    lam1_grid = lam1.reshape(det_xi_grid.shape)
        
    return rays


rays = generateRays(1101014030, 'MOD4', 100)

plt.figure()
numRays = rays.shape[0]
...
for j in range(numRays):
    ray_cyl = rays[j,:,:]
    plt.plot(ray_cyl[0,:], ray_cyl[2,:], c=plt.get_cmap('viridis')(j*1.0/numRays))
...
for j in range(numRays):
    ray_cyl = rays[j,:,:]
    plt.plot(ray_cyl[0,:]*np.cos(ray_cyl[1,:]), ray_cyl[0,:]*np.sin(ray_cyl[1,:]), c=plt.get_cmap('viridis')(j*1.0/numRays))

r0 = 0.67+0.21
r1 = 0.67-0.21
theta = np.linspace(0,2*np.pi)

plt.plot(r0*np.cos(theta), r0*np.sin(theta), c='b')
plt.plot(r1*np.cos(theta), r1*np.sin(theta), c='b')
