...
zeff = eg.calculate_zeff_neo(ne_fit, te_fit, ti_fit, ft_method='lin-liu95', return_funcs=False)
print 'zeff', zeff
ip_func, roa_func = eg.calculate_zeff_neo(ne_fit, te_fit, ti_fit, ft_method='lin-liu95', return_funcs=True)
...
roa = data.variables['RMNMP'].data/eg.e.getAOut()[eg.tind]/100.0
time = data.variables['TIME'].data

tind = np.searchsorted(time, result['time'][i])

volts = data.variables['V'].data[tind,:]
...
ftu = data.variables['NCFTPLUS'].data[tind,:]
zeffc = data.variables['ZEFFC'].data[tind]
zeffp = data.variables['ZEFFP'].data[tind,:]

nuste = data.variables['NUSTE'].data[tind,:]
nusti = data.variables['NUSTI'].data[tind,:]

cloge = data.variables['CLOGE'].data[tind,:]
clogi = data.variables['CLOGI'].data[tind,:]

te = data.variables['TE'].data[tind,:]
ne = data.variables['NE'].data[tind,:]

def resample_func(y):
    return np.interp(roa_func(eg.psigridEv), roa, y)

roa_plot = resample_func(roa)

# %%

transp_sig_neo = resample_func(100.0 / eta_snc)
transp_sig_spitzer = resample_func(100.0 / eta_sps)
transp_ft = resample_func(ft)
transp_lnLe = resample_func(cloge)

test_dict = {}
ip_func(zeffc, test_vars=test_dict)
plt.plot(roa_plot, (test_dict['sig_neo']-transp_sig_neo)/transp_sig_neo, label='new calc, full')

test_dict = {'ft': np.sqrt(roa_plot*eg.e.getAOut()[eg.tind]/eg.e.getMagR()[eg.tind])}
ip_func(zeffc, test_vars=test_dict)
plt.plot(roa_plot, (test_dict['sig_neo']-transp_sig_neo)/transp_sig_neo, label='old trapped e estimate')

test_dict = {'lnLe': transp_lnLe}
ip_func(zeffc, test_vars=test_dict)
plt.plot(roa_plot, (test_dict['sig_neo']-transp_sig_neo)/transp_sig_neo, label='lnLe from TRANSP')

#test_dict = {'lnLe': transp_lnLe, 'ft': transp_ft, 'sig_spitzer': transp_sig_spitzer}
#ip_func(zeffc, test_vars=test_dict)
#plt.plot(roa_plot, (test_dict['sig_neo']-transp_sig_neo)/transp_sig_neo, label='new calc, only Sauter')
