...
PRO zeff_neo_test,shot,zeff,times,zion=zion,zeff_max=zeff_max,n_zeff=n_zeff,dt=dt,trange=trange,plot=plot,verb=verb,ts=ts,efit=efit,qfit=qfit
	...
	rvec = fltarr(efit.nr)+1
 	tvec = fltarr(efit.nt)+1   
   	majr = efit.rout#rvec
    	...
    	eps = minr/majr
    	ft = sqrt(2*eps)   ;approximate trapping fraction
