...
class EquilibriumGeometry:
    ...
    def calculate_zeff_neo(self, ne_fit, te_fit, ti_fit, ft_method='lin-liu95', bayesian=False, mcmc=False):
        """
        ...
        def jllb_func(z, te_unc=1.0, verbose=False):
            ...
            if verbose:
                ...
                print np.min(sig_neo)
                #print ','.join(map(str, roa_func(self.psigridEv)))
                #print ','.join(map(str, sig_spitzer))
                #print ','.join(map(str, lnLe))
                #print ','.join(map(str, lnLe))
                #print l31
            
            return -(sig_neo*fsa_ellb*1.07 + j_bs)
        
        def ip_func(z, te_unc=1.0, verbose=False):
            fsa_dp = (fsa_ne * (fsa_dte + fsa_dti / z) + fsa_dne * ((fsa_te*te_unc) + fsa_ti / z)) * ev2J
            
            fsa_jllb = jllb_func(z, te_unc, verbose)
        ...
        fs_ft = self.fs_ft
        ip_func(1.10096348545, verbose=True)
        fs_ft = np.sqrt(fs_eps)
        ip_func(1.10096348545, verbose=True)
        ...
        if not bayesian:
            ...
            te_fit_unc = te_fit['err_y'][0]/te_fit['y'][0]
            print te_fit_unc
            
            def lnprior(theta):
                te_unc, zeff = theta
                if te_unc <= 0 or zeff <= 1 or zeff > 10:
                    return -np.inf
                else:
                    return -10*(np.log(te_unc/te_fit_unc)/2)**2 + np.log(zeff-1)
            ...
            def lnlike(theta):
                ...
                if te_unc <= 0 or zeff <= 1 or zeff > 10:
                    ...
                    ip_calc = ip_func(zeff, te_unc=te_unc)
                    return -((ip - ip_calc)/ip_std/2)**2
            ...
            def lnprob(theta):
                ...
                if not np.isfinite(lp):
                    ...
                    ip_calc = ip_func(zeff, te_unc=te_unc)
                    if np.isfinite(ip_calc):
                        return lp - ((ip - ip_calc)/ip_std/2)**2
                    else:
                        print te_unc, zeff
                        return -np.inf
            
            nll = lambda *args: -lnlike(*args)
            result = scipy.optimize.minimize(nll, [1.0, 2.0])
            r0 = np.array([0,1])
            
            print ip
            print ip_std
            ndim, nwalkers = 2, 128
            pos = [np.exp(np.random.randn(ndim)*1e-2)*(result['x']-r0)+r0 for i in range(nwalkers)]
            
            sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob)
            sampler.run_mcmc(pos, 1024)
            
            samples = sampler.chain[:, 256:, :].reshape((-1, ndim))
            
            corner.corner(samples, labels=['te_unc', 'zeff'])
...
zeffs[i] = eg.calculate_zeff_neo(ne_fit, te_fit, ti_fit, ft_method='lin-liu95', bayesian=False)
print result['time'][i], 'zeff = '+str(zeffs[i])

zeffs[zeffs<1] = np.min(zeffs[zeffs>1])
result['zeffs'] = zeffs
pkl.dump(result, open('/home/normandy/git/psfc-misc/Fitting/fitted_%d.pkl'%shot, 'w'), protocol=pkl.HIGHEST_PROTOCOL)
...
print np.sum(eg.fs_average(lambda R,Z,psi: R**-1) * eg.fs_vprime * eg.dpsigridEv / 2 / np.pi)
print np.pi * 0.21**2 * 1.8

# %%

"""
rf = np.searchsorted(result['ne_fits'][0]['X'], 0.75)

plt.figure()
for i in range(len(result['time'])):
    plt.plot(result['ne_fits'][i]['X'][:rf], result['ti_fits'][i]['a_Ly'][:rf], c=cm.plasma_r(result['time'][i]-0.5), label=str(result['time'][i]))
plt.legend(loc='upper left')
"""

# %%

plt.plot(result['te_fits'][i]['X'], result['te_fits'][i]['y']*1e3)