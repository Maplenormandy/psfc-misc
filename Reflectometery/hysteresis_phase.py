...
def phiVar(x, t):
    rot = np.exp(-1j*( x[0]*szt2[t,:] + x[1]*(szt2[t,:]**2) ))
    newZ = szr[t,:]*rot
...
for i in range(szr.shape[0]):
    sol = optimize.minimize(phiVar, np.array([0.0, 0.0]), args=(i,))
...
# %% Time-average and plot fluctuations
print csr.shape
...
nlData = np.interp(vtime, nltime, nl04Node.data())/1e20
fluctData = np.interp(vtime, cst2, csr2)

plt.scatter(nlData[locInds], fluctData[locInds], c='r', label=str(shot), marker='^')
plt.scatter(nlData[socInds], fluctData[socInds], c='b', label=str(shot), marker='v')
